...
The extension keeps BSAM source, decks, meshes, artifacts, tool results, and the registry parameter catalog local. An optional OpenAI Responses adapter can route the user's typed chat request under the `sanitized` policy with `store: false`; it cannot read repository files. Registry-derived parameter forms, reviewed diffs, confirmation-gated apply/run/stop controls, and a dedicated guarded chat window are available inside VS Code. See the [OpenAI provider setup](docs/guides/OPENAI_PROVIDER_SETUP.md).
