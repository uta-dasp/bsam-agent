...
- `BSAM Agent: Open Chat Window` opens a dedicated, theme-aware chat panel beside the editor. A private JSON-lines bridge reuses the same local provider configuration, saved conversation state, digest audit, reviewed diffs, and separate confirmation policy as the CLI. An expandable task card shows the current objective, plan, completed tool activity, bounded evidence summaries, assumptions, hypotheses, completion criteria, and terminal reason. Model and task text are rendered through DOM `textContent`, never as HTML.
