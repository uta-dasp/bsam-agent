...
- `BSAM Agent: Open Chat Window` opens a dedicated, theme-aware chat panel beside the editor. A private JSON-lines bridge reuses the same local provider configuration, saved conversation state, digest audit, reviewed diffs, and separate confirmation policy as the CLI. Model text is rendered as escaped plain text.
