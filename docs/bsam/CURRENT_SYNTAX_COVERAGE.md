...
Registry version `0.18.0` records 13 active top-level blocks, 29 finite-element cluster command dispatches, 12 active nested BOUNDARY constructs, two runtime-verified transformations, five obsolete/compatibility tokens, and 70 local evidence records. It defines structured bodies and dependencies for all 29 FE commands plus the complete SOLVER, UFUNCTIONS, USER, CRACK, CONSTITUTIVE, FAILURE, TABLES, STATISTICAL, and externally bounded MOISTURE grammars; all 28 active MATERIALS types and the structured 50, 998, and 999 forms; and BOUNDARY solver scheduling, connections, loading sequences, convergence controls, and output requests. Unsafe or ineffective FE source paths are explicitly preservation-only or blocked. The FE include reader uses a nested unit stack, prepends the original BSAM input directory for every FILE target, and resumes the parent stream at included-file EOF. The Agent preserves this source graph while additionally blocking cycles and workspace escapes. The Agent API exposes transformation and obsolete-token rules with the remaining capability manifest. The registry generates the [BSAM 2.4 current input API reference](reference/BSAM_2_4_INPUT_API.md). The baseline transition and reproducibility qualification are recorded in [the 2026-08-31 audit](BASELINE_AUDIT_2026-08-31.md).
...
| Materials | `source/libbsam/mat_ini.f90`, `material.f90`, `interface_material.f90` | All active types inventoried; structured 50/998/999 bodies documented; legacy bodies pending | Complete |
