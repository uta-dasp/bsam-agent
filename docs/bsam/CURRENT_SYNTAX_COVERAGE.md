...
Parameter-level `edit_operations` are fail-closed independently of construct maturity. Registry 0.74.0 verifies optional `maxiterations` record insertion/removal, `DAMP` flag token insertion/removal, and SHEFF `relative_tolerance` option-row insertion/removal; unmarked parameter shape changes remain unavailable.
